...
if (isset($_POST['call_now'])){
    ...
    foreach($articles as $key => $value){
        ...
        $articles_html.= '<div class="col-md-6" style="text-align: right;">'; $articles_html.= '<form action="news_edit.php" method="POST">';
        ...
        $articles_html.= '</div>';
        $articles_html.= '</div>';
        $articles_html.= '<div class="card-footer text-muted">'; $articles_html.= strftime('%A %e %B %Y %H:%M', $value['date']);
        $articles_html.= '</div>'; $articles_html.= '</div>';
    }
